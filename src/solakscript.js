...
function init() {
  ...
  if (manualCompleteBtn) {
    var doManualComplete = async function() {
      ...
      try {
        var result = await sb.rpc('solak_admin_add_to_completed', { pass: adminPass, player_name: name });
        if (result.error) throw result.error;
        ...
        console.warn('[SLK] Manual complete failed:', err);
        manualCompleteBtn.textContent = '!';
  ...
  if (manualQueueBtn) {
    manualQueueBtn.addEventListener('click', async function() {
      ...
      try {
        var result = await sb.rpc('solak_admin_add_to_queue', { pass: adminPass, player_name: name });
        if (result.error) throw result.error;
        if (manualCompleteInput) manualCompleteInput.value = '';
        ...
        console.warn('[SLK] Manual queue add failed:', err);
        manualQueueBtn.textContent = '!';
  ...
  async function attemptCalibration() {
    ...
    try {
      var result = await sb.rpc('login_admin', {
        ...
        p_required_roles: ['solak_gm', 'admin', 'super_admin']
      });
      if (result.error) throw result.error;
  ...
  if (flushSkippedBtn) {
    flushSkippedBtn.addEventListener('click', async function() {
      ...
      } catch(err) { console.warn('[SLK] flush skipped failed:', err); flushSkippedBtn.textContent = 'Flush'; flushSkippedBtn.disabled = false; }
    });
  }
