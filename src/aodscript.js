/* ================================================================
   AoD GM Timer — aodscript.js
   ================================================================
   Manages the AoD GM carry queue using Supabase as the backend.
...
async function fetchQueue() {
  try {
    var result = await sb.from('aod_queue').select('name').order('joined_at');
    if (result.error) throw result.error;
    queueData = result.data ? result.data.map(function(r) { return r.name; }) : [];
    return queueData;
  } catch (err) {
    console.warn('[AOD] Failed to fetch queue:', err);
    return null;
  }
}
...
async function fetchBlacklist() {
  try {
    ...
    console.warn('[AOD] Failed to fetch blacklist:', err);
  }
}

async function fetchInfoCode() {
  try {
    var result = await sb.from('aod_app_state').select('value').eq('key', 'info_code').single();
    if (result.error) throw result.error;
    INFO_CODE = (result.data.value || '').toLowerCase().trim();
  } catch (err) {
    console.warn('[AOD] Failed to fetch info code:', err);
  }
}

async function fetchSubmissionsOpen() {
  try {
    var result = await sb.from('aod_app_state').select('value').eq('key', 'submissions_open').single();
    if (result.error) throw result.error;
    submissionsOpen = result.data.value === 'true';
  } catch (err) {
    console.warn('[AOD] Failed to fetch submissions status:', err);
...
async function fetchWorld() {
  try {
    var result = await sb.from('aod_app_state').select('value').eq('key', 'world').single();
    if (result.error) throw result.error;
    ...
    console.warn('[AOD] Failed to fetch world:', err);
  }
}

async function fetchStats() {
  try {
    var result = await sb.from('aod_app_state').select('key, value').in('key', ['total_kills', 'session_kills']);
    if (result.error) throw result.error;
    var total = '—';
    var today = '—';
    ...
    console.warn('[AOD] Failed to fetch stats:', err);
  }
}

async function fetchCompleted() {
  try {
    var result = await sb.from('aod_completed').select('name').order('id', { ascending: false });
    if (result.error) throw result.error;
    ...
    console.warn('[AOD] Failed to fetch completed:', err);
  }
}
...
async function fetchHeartbeats() {
  try {
    ...
    console.warn('[AOD] Heartbeat fetch failed:', err);
  }
}
...
async function sendHeartbeat() {
  ...
  try {
    ...
    console.warn('[AOD] Heartbeat send failed:', err);
  }
}
...
async function fetchPings() {
  ...
  try {
    ...
    if (result.data && result.data.length > 0) {
      playAlert('turn');
      console.log('[AOD] Ping received!');
    }
    lastPingCheck = new Date().toISOString();
  } catch (err) {
    console.warn('[AOD] Ping fetch failed:', err);
  }
}

async function fetchSessionState() {
  try {
    var result = await sb.from('aod_app_state').select('key, value').in('key', ['session_kills', 'session_active']);
    if (result.error) throw result.error;
    for (var i = 0; i < result.data.length; i++) {
...
async function fetchSkipped() {
  try {
    var result = await sb.from('aod_skipped').select('name').order('skipped_at', { ascending: false }).limit(20);
    if (result.error) throw result.error;
    skippedData = result.data ? result.data.map(function(r) { return r.name; }) : [];
    renderSkippedPanel();
  } catch (err) {
    console.warn('[AOD] Skipped fetch failed:', err);
  }
}
...
function updateStatus(queue) {
  ...
  if (idx === 0) {
    ...
    alertSub.innerHTML     = (currentWorld
      ? 'Head to AoD on World: ' + currentWorld + ' now!'
      : 'Head to AoD now!')
      + '<br>Please be at the entrance ready to fight AoD';
...
async function runAction(action, name, btnEl) {
  ...
  try {
    var result;
    if (action === 'adminDone') {
      result = await sb.rpc('aod_admin_complete', {
        pass: adminPass,
        player_name: name,
        increment_session: sessionActive
      });
    } else if (action === 'adminSkip') {
      result = await sb.rpc('aod_admin_skip', { pass: adminPass, player_name: name });
    } else if (action === 'ping') {
    ...
    console.warn('[AOD] Action failed:', err);
    btnEl.textContent = '!';
...
function setupRealtime() {
  sb.channel('aod-realtime')
    .on('postgres_changes', { event: '*', schema: 'public', table: 'aod_queue' }, onRealtimeChange)
    .on('postgres_changes', { event: '*', schema: 'public', table: 'aod_app_state' }, onRealtimeChange)
    .on('postgres_changes', { event: '*', schema: 'public', table: 'aod_skipped' }, onRealtimeChange)
    .on('postgres_changes', { event: '*', schema: 'public', table: 'blacklist' }, function() { fetchBlacklist(); })
    .on('postgres_changes', { event: '*', schema: 'public', table: 'aod_completed' }, function() {
      fetchCompleted();
      onRealtimeChange();
    })
...
async function fetchAdminChat() {
  try {
    ...
    console.warn('[AOD] Failed to fetch chat:', err);
  }
}
...
async function sendChatMessage() {
  ...
  var sender = adminName || localStorage.getItem('admin_display_name') || getEffectiveName() || 'Admin';
  ...
  try {
    ...
    console.warn('[AOD] Send chat failed:', err);
  }
...
function unbanPlayer(name) {
  ...
  sb.rpc('super_admin_remove_blacklist', { pass: superAdminPass, player_name: name })
    .then(function(res) {
      if (res.error) console.warn('[AOD] Unban failed:', res.error);
      else fetchBlacklist();
    });
}
...
async function joinQueue() {
  ...
  try {
    var result = await sb.rpc('aod_join_queue', { player_name: name });
    if (result.error) throw result.error;
    ...
    console.warn('[AOD] Join queue error:', e);
...
function init() {
  try {
    var saved = localStorage.getItem('aod_playerName');
    if (saved && saved.trim()) {
  ...
  var savedReminder = localStorage.getItem('aod_remindersEnabled');
  ...
  reminderToggle.addEventListener('change', function() {
    ...
    localStorage.setItem('aod_remindersEnabled', this.checked);
  });
  ...
  function switchToTab(tabName) {
    ...
    try { localStorage.setItem('aod_activeTab', tabName); } catch (e) {}
  ...
  var savedTab = urlTab || localStorage.getItem('aod_activeTab') || 'info';
  ...
  document.getElementById('name-override').addEventListener('input', function() {
    try { localStorage.setItem('aod_playerName', this.value.trim()); } catch (e) {}
  ...
  function activateAdmin(pass, displayName) {
    ...
    try {
      ...
      if (displayName) localStorage.setItem('admin_display_name', displayName);
    } catch (e) {}
  ...
  if (blacklistAddBtn) {
    blacklistAddBtn.addEventListener('click', function() {
      ...
      sb.rpc('super_admin_add_blacklist', { pass: superAdminPass, player_name: name })
        .then(function(res) {
          if (res.error) console.warn('[AOD] Ban failed:', res.error);
  ...
  if (manualCompleteBtn) {
    var doManualComplete = async function() {
      ...
      try {
        var result = await sb.rpc('aod_admin_add_to_completed', { pass: adminPass, player_name: name });
        if (result.error) throw result.error;
        ...
        console.warn('[AOD] Manual complete failed:', err);
        manualCompleteBtn.textContent = '!';
  ...
  if (manualQueueBtn) {
    manualQueueBtn.addEventListener('click', async function() {
      ...
      try {
        var result = await sb.rpc('aod_admin_add_to_queue', { pass: adminPass, player_name: name });
        if (result.error) throw result.error;
        if (manualCompleteInput) manualCompleteInput.value = '';
        ...
        console.warn('[AOD] Manual queue add failed:', err);
        manualQueueBtn.textContent = '!';
  ...
  try {
    ...
    var savedName = localStorage.getItem('admin_display_name');
    ...
    if (savedPass && savedExpiry && Date.now() < Number(savedExpiry)) {
      sb.rpc('check_admin', { pass: savedPass }).then(function(result) {
        if (result.data === true) {
          activateAdmin(savedPass, savedName);
  ...
  async function attemptCalibration() {
    ...
    if (!enteredName || !enteredPass) {
      adminError.textContent = 'Enter your name and passcode';
    ...
    try {
      var result = await sb.rpc('login_admin', {
        ...
        p_required_roles: ['aod_gm', 'admin', 'super_admin']
      });
      if (result.error) throw result.error;
      if (result.data && result.data.valid) {
        ...
        adminError.textContent = 'Invalid name or passcode';
        adminError.style.display = 'block';
      }
    } catch (err) {
  ...
  queueListEl.addEventListener('click', function(e) {
    ...
    if (nameEl && !nameEl.classList.contains('editing')) {
      ...
      input.setAttribute('name', 'aod-edit-' + Date.now());
      ...
      function saveEdit() {
        ...
        if (newName && newName !== original) {
          ...
          sb.rpc('aod_admin_edit_name', { pass: adminPass, old_name: original, new_name: newName })
            .catch(function(err) { console.warn('[AOD] Edit failed:', err); });
        } else {
  ...
  queueListEl.addEventListener('drop', function(e) {
    ...
    sb.rpc('aod_admin_reorder_queue', { pass: adminPass, player_name: movedName, new_position: targetIdx })
      .then(function(res) {
        if (res.error) { console.warn('[AOD] Reorder failed:', res.error); refresh(); }
      })
      .catch(function(err) { console.warn('[AOD] Reorder failed:', err); refresh(); });
  });
  ...
  document.getElementById('ac-toggle-submissions').addEventListener('click', async function() {
    ...
    btn.disabled = true;
    try {
      var result = await sb.rpc('aod_admin_toggle_submissions', { pass: adminPass });
      if (result.error) throw result.error;
      await fetchSubmissionsOpen();
      updateToggleOpenBtn();
      updateSubmissionsStatus();
    } catch (err) {
      console.warn('[AOD] Failed to toggle open:', err);
    }
    btn.disabled = false;
  });
  ...
  document.getElementById('ac-session-start').addEventListener('click', async function() {
    ...
    this.disabled = true;
    try {
      var result = await sb.rpc('aod_admin_session_start', { pass: adminPass });
      if (result.error) throw result.error;
      sessionActive = true;
      sessionKillCount = 0;
      updateSessionDisplay();
    } catch (err) {
      console.warn('[AOD] Session start failed:', err);
    }
    this.disabled = false;
  });
  ...
  document.getElementById('ac-session-end').addEventListener('click', async function() {
    ...
    this.disabled = true;
    try {
      var result = await sb.rpc('aod_admin_session_end', { pass: adminPass });
      if (result.error) throw result.error;
      sessionActive = false;
      updateSessionDisplay();
    } catch (err) {
      console.warn('[AOD] Session end failed:', err);
    }
    this.disabled = false;
  });
  ...
  document.getElementById('ac-world-input').addEventListener('input', function() {
    ...
    worldDebounce = setTimeout(function() {
      ...
      var worldInput = document.getElementById('ac-world-input');
      sb.rpc('aod_admin_set_world', { pass: adminPass, new_world: val })
        .then(function(res) {
          if (res.error) {
            console.warn('[AOD] World update failed:', res.error);
  ...
  document.getElementById('completed-side-list').addEventListener('click', function(e) {
    ...
    if (skipBtn && !skipBtn.disabled) {
      ...
      skipBtn.textContent = '...';
      sb.rpc('aod_admin_uncomplete_to_skip', { pass: adminPass, player_name: skipName })
        ...
        .catch(function(err) {
          console.warn('[AOD] Uncomplete to skip failed:', err);
    ...
    function saveEdit() {
      ...
      if (newName && newName !== original) {
        ...
        nameEl.classList.remove('editing');
        sb.rpc('aod_admin_edit_completed_name', { pass: adminPass, old_name: original, new_name: newName })
          .catch(function(err) { console.warn('[AOD] Completed edit failed:', err); });
      } else {
        nameEl.textContent = original;
        nameEl.classList.remove('editing');
      }
  ...
  document.getElementById('chat-messages').addEventListener('click', async function(e) {
    ...
    btn.disabled = true;
    try {
      var r = await sb.rpc('delete_admin_chat', { pass: adminPass, p_id: id });
  ...
  if (flushSkippedBtn) {
    flushSkippedBtn.addEventListener('click', async function() {
      ...
      } catch(err) { console.warn('[AOD] flush skipped failed:', err); flushSkippedBtn.textContent = 'Flush'; flushSkippedBtn.disabled = false; }
    });
  }
  ...
  document.getElementById('skipped-list').addEventListener('click', async function(e) {
    ...
    if (unskipBtn && !unskipBtn.disabled) {
      ...
      try {
        var result = await sb.rpc('aod_admin_unskip', { pass: adminPass, player_name: name });
        if (result.error) throw result.error;
        skippedData = skippedData.filter(function(n) { return n !== name; });
        renderSkippedPanel();
        onRealtimeChange();
      } catch (err) {
        console.warn('[AOD] Unskip failed:', err);
    ...
    if (completeBtn && !completeBtn.disabled) {
      ...
      try {
        var result = await sb.rpc('aod_admin_skip_to_complete', { pass: adminPass, player_name: cName });
        if (result.error) throw result.error;
        skippedData = skippedData.filter(function(n) { return n !== cName; });
        renderSkippedPanel();
        onRealtimeChange();
      } catch (err) {
        console.warn('[AOD] Skip to complete failed:', err);
...
if (typeof alt1 !== 'undefined') {
  try {
    alt1.identifyAppUrl('./aodappconfig.json');
  } catch (e) {
    console.error('[AOD] identifyAppUrl error:', e);
  }
} else {
